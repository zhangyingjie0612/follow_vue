    ...
    <el-col :offset="1" :span="22">
      <el-table
        ...
        <el-table-column>
          ...
          <el-table-column prop="deptId" label="部门编号" width="250" align="center">
          </el-table-column>
          <el-table-column prop="deptName" label="部门名称" width="250" align="center">
          </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkIfExist(ids){
        axios.get('/dept/checkIfExist/'+ids).then(res=>{
          if (res.data!=null){
      ...
      batchDeleteByIds(){
        ...
        //console.log(ids)
        axios.get('/dept/batchDeleteByIds/'+ids).then(res=>{
          if(res.data>0){
            alert("删除成功");
          }else{
            alert("删除失败");
        ...
        setTimeout(()=>{
          if (this.ifExist){
            ...
            this.$confirm('确认删除该记录吗?', '提示', {
              ...
              /*考虑假删*/
              axios.get("/dept/deleteByIdFake/"+this.deleteId).then(res=>{
                if (res.data > 0) {
                  this.$message({
                    message: '删除成功',
                    type: 'success'
